...
- **source·target 노드의 NVIDIA 드라이버 버전이 동일**해야 한다(예: `570.211.01`).
  - cuda-checkpoint 복원은 드라이버 버전이 맞아야 동작하고,
...
- target 노드에 이 시스템 설치 완료: patch crio + `scripts/install-node.sh`
  (hook + **restore-agent** + host helper) + 인터셉터 lib + device plugin.
...
`nfs://<server>/<export>/<file>.tar` 또는 미리 `scp` 후 `hostpath://` 도 지원한다.
...
restore-agent가 자동으로 cuda-checkpoint 제어상태 복원 + 인터셉터 데이터 remap을 수행한다.
수동 명령 없이 완료된다.
